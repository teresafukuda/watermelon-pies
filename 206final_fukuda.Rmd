...
```
*Figure 1. Model of Taylor Manifest Anxiety Scale by age.* 
...
“Pesticide use of P yraclostrobin differed between the two crops, with a higher median sample rank (value) for carrots (n =  N), versus a sample median of VALUE  for strawberries (n = NNNNN). A Mann-Whitney U test revealed that the difference was significant (W = `r pesticide_test$statistic`, p = `r pesticide_test$p.value`) with a moderate effect size (Cliff’s delta = `r pesticide_cliffs$estimate`)."
