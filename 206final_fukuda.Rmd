...
*Table 1.* Multiple linear regression model of opinions on climate change. 
...
b. There is a significant association between racial groups and perceived respect on UCSB campus; different racial groups feel significantly differently about the level of respect that they receive. 
...
a. **Table 1. Self consciousness (Taylor Manifest Anxiety Scale) binomial logarithmic regression results.** Data source: openpsychometrics.org
...
c. Between both sexes, the graph shows a decrease in the probability of self-reported self sonciousness with age. This is consistent with the results in (a) where the logodds of self consciousness has a negative coefficient with a significant p-value. Also there is a very small, almost imperceptible difference between males and females on the graph, which is consistent with the very small decrease in logodds predicted by the model between males and females with an p value > 0.05. 
